...
export default class ChangePin extends Component {
  ...
  onChangePin = async () => {
    ...
    let confirmpin = "";
    let user_id = data.id;
    console.log("Old pin", this.state.oldPin);
    ...
    if (this.state.oldPin.length == 4) {
      ...
      if (this.state.newPin.length == 4) {
        ...
        if (this.state.confirmPin.length == 4) {
          pin = parseInt(this.state.oldPin);
      ...
      this.setState({ ...this.state, loading: false });
      Toast.show("Old pin is invalid", { duration: Toast.durations.LONG });
    }
  };
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <FourDigitsPin
          text={"Insert Old Pin"}
          setPinValue={(pin) => this.setState({ ...this.state, oldPin: pin })}
        />
